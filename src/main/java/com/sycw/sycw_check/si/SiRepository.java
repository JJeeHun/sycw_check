...
public interface SiRepository {
    ...
    @Select("select * from tb_po0801 where SYSTEM_ORDER_NO in (select tb_po0800.SYSTEM_ORDER_NO from tb_po0800 where CONTACT_ORD_NO = #{contactOrderNo})")
    List<OrderItemEntity> findOrderItem(String contactOrderNo);

    @Select("select * from tb_oo0201 where CONTACT_ORD_NO = #{contactOrderNo} order by SI_NO,SI_ITEM_NO,ETD")
    ...
    @Select("select * from tb_oo0400 where CONTACT_ORD_NO = #{contactOrderNo} order by SI_NO,SI_ITEM_NO,ETD")
