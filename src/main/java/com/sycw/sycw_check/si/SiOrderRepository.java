...
@Repository
@Mapper
public interface SiOrderRepository {
    ...
    @Select("select * from tb_oo0201 where CONTACT_ORD_NO = #{contactOrderNo} order by SI_NO ,SI_ITEM_NO")
    List<ShippingEntity> findShipping(String contactOrderNo);

    @Select("select * from tb_oo0300 where CONTACT_ORD_NO = #{contactOrderNo} order by SI_NO ,SI_ITEM_NO")
    List<PickingPlanEntity> findPlan(String contactOrderNo);

    @Select("select * from tb_oo0400 where CONTACT_ORD_NO = #{contactOrderNo} order by SI_NO ,SI_ITEM_NO")
    List<PickingEntity> findPicking(String contactOrderNo);

    @Select("select * from tb_oo0500 where CONTACT_ORD_NO = #{contactOrderNo} order by SI_NO ,SI_ITEM_NO")
    List<ClosingEntity> findClosing(String contactOrderNo);

    @Select("select * from tb_su753d where CONTACT_ORD_NO = #{contactOrderNo} order by SI_NO,ITEM_NO")
    List<Sending753Entity> findSending753(String contactOrderNo);

    @Select("select * from tb_su856d where CONTACT_ORD_NO = #{contactOrderNo} order by SI_NO,SEQ")
